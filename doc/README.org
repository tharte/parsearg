...
\newpage
* Worked Example: Let's build the interface for a TO-DO app 
...
\begin{remark}
The definition implies that a tree ``points'' to its children, in so far
as the tree is necessarily aware of the children it contains, whereas its
children are not necessarily aware of their parent.  We will therefore regard
this definition as the ``top-down'' definition.
\end{remark}
