...
#+END_SRC
If we use ~argparse~, adding the subprogram ~create~ would mean having to
explicitly add a subparser.  With ~parsearg~, the CLI for the above is declared
...
#+caption: Construct parser with ~argparse~
...
#+caption: Construct parser with ~parsearg~
...
# the CLI "view" comprises pure data: the parser is fully specified by this view - no imperative instructions are required
view = {
    'create|user': {
