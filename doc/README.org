...
\begin{remark}
The definition implies that a tree ``points'' to its children, in so 
far as the tree is aware of its children, whereas its children are
...
~e1~, ~e2~, ~...~, is itself a tree.  The empty tree is represented by
#+begin_quote
~(None, {})~,
#+end_quote
