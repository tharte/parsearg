...
Any node of a tree is identified by a value and a set of children in a
/recursive/ way.  Indeed, all trees are recursive data structures.
...
We represent the value/node/name and its set of children as the tuple: 
#+begin_quote
~(value, {e1, e2, ....})~,
#+end_quote
where ~{e1, e2, ...}~ is the set of children, and each element of the set
~e1~, ~e2~, and so on, is itself a tree.  
The empty tree is represented by
#+begin_quote
~(None, {})~,
#+end_quote
where ~None~ represents no value, and ~{}~ is the empty set.
...
an n-ary tree, which has the same number of children at each node 
...
We can create a ``longhand'' version of the A tree using the 
~Tree~ class, but it is cumbersome to use and---more specifically---it can easily obfuscate the
tree structure itself when the node names and payloads are in any way complex.[fn:: For clarity the nodes' payloads, /i.e./ the nodes' values, are omitted.] 
#+BEGIN_SRC python :session *Python-3.x* :exports both :results output :eval yes 
"""
       A
     / | \
...
This representation is a ``flat tree'' or a ``flattened tree''. Each string
represents a node in the tree and names that node. 
...
Leaf nodes are terminating nodes in the tree's hierarchy. 
In the above, the terminating nodes are
...
** Unflattening the flattened tree
~parsearg~ transforms a ~dict~---the keys of which are shorthand for the
tree's nodes and the values of which are the nodes' payloads---into a 
~Tree~. We need the shorthand of the ~dict~ for ease of specification, 
and we need the ~Tree~ to avail of the data structure's properties. 

First, ~parsearg~ defines a ~Key~. Take a key that represents a leaf
node ~'A|BB|C'~:
#+BEGIN_SRC python :session *Python-3.x* :exports both :results output :eval yes 
key = 'A|BB|C'
print( Key(key) )
...
along with a ~Node~, which is a representation of the key's delimited string
as a nested list:
#+BEGIN_SRC python :session *Python-3.x* :exports both :results output :eval yes 
print( Key(key).value )
...
What is a ~Node~? It is a tuple that has a ~head~ and a ~tail~. The 
~head~ is the name of the node and the ~tail~ is a nested list containing
the remainder of the leaf node's specification:
#+BEGIN_SRC python :session *Python-3.x* :exports both :results output :eval yes 
...
Each ~Key~ can also store a ~payload~, but it is ~None~ here:
#+BEGIN_SRC python :session *Python-3.x* :exports both :results output :eval yes 
payload = 3.141593
d = {key: payload}
...
but the ~key~ is preserved. A ~Node~, upon which the ~Key~ is based, can also 
be shifted:
#+BEGIN_SRC python :session *Python-3.x* :exports both :results output :eval yes 
print( Node(head='A', tail=['B', ['C']]) )
print( Node(head='A', tail=['B', ['C']]) << 1 )
print( Node(head='A', tail=['B', ['C']]) << 2 )
print( Node(head='A', tail=['B', ['C']]) << 3 )
#+END_SRC
...
** Heterogenous trees: recursive data structures
At any level, there is a sense in which a node is identified by 
its parent in a /recursive/ way. Indeed, trees are recursive data structures. 
...
An heterogeneous tree is either:
    ...
    1. a value (or ``node'', or ``name'') and a ~set~ (possibly empty)
       of child trees (the ``children'' of the value/node/name); or,
    2. the empty tree.

We represent the value/node/name and its set of children as the tuple: 
~(value, {e1, e2, ....})~,
where ~{e1, e2, ...}~ is the set of children and each element of the 
set ~e1~, ~e2~, and so on, is itself a tree.
The empty tree is represented by ~(None, {})~.
       
This type of tree is termed /heterogeneous/ because each node's set of 
child trees can vary in its cardinality, whereas an n-ary tree has the
same number of children at each node 
(/e.g./ two for a binary tree, three for a ternary tree, and in general 
$n$ for an n-ary tree).
...
#+end_quote
Note that, by this definition, the empty tree (represented by ~(None, {})~) is 
a terminating node, because nothing in the definition requires its parent 
node to exist.
In this light, ~(None, {})~ is a perfectly valid leaf node, as is
~(value, {(None, {})})~.
       
We can define an heterogeneous tree in reverse, /viz./ 
first from its terminating nodes. To do so
requires that we enrich the existing terminology to avert linguistic gymnastics.
``Siblings'' are the children of a particular node, namely the siblings' ``parent''.
This is nothing more than the flip side of the above ``top-down'' definition of an 
heterogeneous tree. 

Therefore, an heterogeneous tree can also be defined as being either:

    1. a ~set~ (possibly empty) of child trees having a common parent 
       (being a value/node/name); or,
    2. the empty tree.
       
We will regard this as the ``bottom-up'' definition. 

** Creating a tree from its leaf nodes
