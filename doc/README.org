...
~(None, {None})~,
#+end_quote
where ~None~ represents no value, and ~{}~ is the empty set of children.[fn:: Python has a quirk where ~{}~ is of type ~dict~, whereas ~{None}~ is of type ~set~. Thus, the ``empty set'' is really represented by ~{None}~ and not ~{}~.] 
