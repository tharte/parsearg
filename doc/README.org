...
names a node in the tree. At any level of the tree, each node's
/children/ must contain a unique set of node names, 
otherwise the tree's structure would be indeterminate. 
The example of the A-AA tree illustrates that the structure of 
the A tree can be duplicated into the AA tree; only the root
nodes of each sub-tree (A and AA, respectively) need to be unique.
Leaf nodes are terminating nodes in the
...
its parent in a /recursive/ way.
#+attr_latex: :width 1.0\linewidth
#+RESULTS:
[[file:./img/tree-abc-a-aa.png]]

Trees are defined recursively. An heterogeneous tree is either:

    1. the empty tree (represented by ~None~), or
    2. a value (represented by ~value~ or ~payload~) and a ~list~ of child trees.
       
To create this structure from its inverse, /viz./ the child nodes (and ultimately
its leaf nodes), requires that we introduce the concept of the /parent/ node
and /sibling/ nodes. Siblings are the children of a node, so they are already
defined and therefore ``sibling'' is synonymous with children. 
       
Inverting this structure from the terminating nodes (the leaf nodes), we have
A leaf node is:

    1. the empty tree (represented by ~None~), or
    2. a tree whose ~list~ of child trees is empty and whose parent node is either non-empty or empty.
