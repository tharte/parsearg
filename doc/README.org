...
#+caption: Parser constructed with ~argparse~: the parser must be specified with imperative instructions
...
\newpage
#+caption: Parser constructed with ~parsearg~: the parser is specified with data only---it is declarative
