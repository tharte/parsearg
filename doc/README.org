...
We represent the value/node/name and its set of children with a tuple: 
where ~{e1, e2, ...}~ is the set of children and each element of the 
set ~e1~, ~e2~, ..., are themselves trees.
...
``Siblings'' are the children of a particular node: the siblings' ``parent''.
It's simply the flip side of the definition of an heterogeneous tree, above. 
...
Inverting this structure from the terminating nodes (the leaf nodes), we have
*TODO*: unfinished here

In other words, ~(None, {})~ is a perfectly valid leaf node, as is
~(value, {(None, {})})~, which reduces to ~(value, {})~.
