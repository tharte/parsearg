...
design pattern. The ~dict~ embeds callbacks within the Controller component,
...
The keys of this ~dict~ form a flattened tree of subcommands. 
~parsearg~ unflattens the flattened tree into
a tree of ~argparse~ parsers. ~parsearg~ requires nothing 
